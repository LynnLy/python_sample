...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("length must match")
        valcounter = 0
        for parameter in formals:
            frame.define(parameter, vals[valcounter])
            valcounter += 1
        return frame
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            val = (self.fn(*pythonlist))
        except TypeError:
            raise SchemeError("Invalid primitives")
        return (val, None)
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if not scheme_symbolp(target.car()):
            raise SchemeError("Can only define symbols")
        lambexp = Pair("lambda", Pair(target.cdr(), vals.cdr()))
        env.define(target.car(), scheme_eval(lambexp, env))
        return (target.car(), None)
    else:
...
def do_let_form(vals, env):
    ...
    "*** YOUR CODE HERE ***"
    for eachbind in bindings:
        names = Pair(eachbind.car(), names)
        values = Pair(scheme_eval(eachbind.cdr().car(), env), values)
...
def do_if_form(vals, env):
    ...
    if not predicate:
        if len(vals) == 3:
            return (scheme_eval(vals.cdr().cdr().car(), env), None)
        else:
            return (scheme_eval(okay, env), None)
    return (scheme_eval(vals.cdr().car(), env), None)
...
def do_and_form(vals, env):
    ...
    for subexp in vals:
        expval = scheme_eval(subexp, env)
        if expval is scheme_false:
...
def do_cond_form(vals, env):
    ...
    for i, clause in enumerate(vals):
        ...
        if test:
            "*** YOUR CODE HERE ***"
            if_true_val = clause.second
            print(if_true_val)
            if if_true_val == nil:
                print('if nil')
                return (test, None)
            elif len(if_true_val) > 1:
                    beginform = Pair("begin" , if_true_val)
                    return (scheme_eval(beginform, env), env)    
            return (scheme_eval(if_true_val.first, env), None)
...
def do_begin_form(vals, env):
    ...
    for eachvalue in vals:
        final = scheme_eval(eachvalue, env)
    return (final, None)
...
def check_formals(formals):
    ...
    formalslist = []
    for parameter in formals:
        if parameter in formalslist:
            raise SchemeError("Repeated symbol")
        if not parameter.symbolp():
            raise SchemeError("Not a symbol")
        formalslist.append(parameter)
