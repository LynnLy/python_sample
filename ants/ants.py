    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant is not None:
                if self.ant.can_contain(insect): #if current ant is a bodyguard
                    self.ant.contain_ant(insect)
                    insect.place = self
                    return
                elif insect.can_contain(self.ant): #if trying to add bodyguard
                    insect.place = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        currentindex = 0
        while currentplace is not hive and currentplace.entrance is not None:
            if currentindex > self.max_range:
                return None
            if currentindex >= self.min_range:
                if len(currentplace.bees) != 0:
                    return random_or_none(currentplace.bees)
            currentplace = currentplace.entrance
            currentindex += 1
        if currentindex >= self.min_range and currentindex <= self.max_range and currentplace is not hive:
                if len(currentplace.bees) != 0:
                    return random_or_none(currentplace.bees)
        return None
...
class QueenPlace:
    ...
    def bees(self):
        listbees = []
        listbees.append(self.colony_queen.bees)
        listbees.append(self.ant_queen.bees)
...
class QueenAnt(ScubaThrower):
    ...
    def __init__(self):
        ...
        if QueenAnt.numberqueens == 0:
            self.imposter = False
            QueenAnt.numberqueens += 1
        else:
            self.imposter = True
            QueenAnt.numberqueens += 1
        QueenAnt.doubledants.append(self)

    def double(self, place):
            if place.ant is not None:
                    if place.ant.container:
                        if place.ant.ant is not None:
                            if place.ant.ant not in QueenAnt.doubledants:
                                place.ant.ant.damage *= 2
                                QueenAnt.doubledants.append(place.ant.ant)
                    if place.ant not in QueenAnt.doubledants:
                        place.ant.damage *= 2
                        QueenAnt.doubledants.append(place.ant)

    def action(self, colony):
        """A queen ant throws a leaf, but also doubles the damage of ants
        in her tunnel.  Impostor queens do only one thing: die."""

        def double(place): #doesn't work for a reason I don't know
            if place.ant is not None:
                    if place.ant.container:
                        if place.ant.ant is not None:
                            if place.ant.ant not in QueenAnt.doubledants:
        ...
        if self.imposter:
            ...
            if self.startplace is None:
                self.startplace = colony.queen
                colony.queen = QueenPlace(self.startplace, self.place) 
            else: 
                colony.queen = QueenPlace(self.startplace, self.place)
            ...
            currentplace2 = self.place.entrance
            while currentplace2 is not None:
                double(currentplace2)
                currentplace2 = currentplace2.entrance
            self.throw_at(self.nearest_bee(colony.hive))
